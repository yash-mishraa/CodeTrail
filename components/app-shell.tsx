  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-40 flex h-[38px] items-center justify-between border-b border-zinc-800 bg-[#000000] font-mono text-[11px] shadow-sm select-none">
        ...
        <div className="flex items-end h-full">
          {tabs.map((tab) => (
            <button
              ...
              className={`h-full px-4 border-r border-zinc-800 flex items-center gap-2 transition-colors group ${currentTab === tab ? "bg-[#0c100e] text-zinc-100 shadow-[inset_0_1px_0_0_#9BFF2E]" : "bg-black text-zinc-500 hover:bg-[#0c100e]"}`}
            >
      ...
      <div className="flex items-center h-full shrink-0">
        <button onClick={() => setPalette(true)} className="flex items-center gap-2 h-full px-4 border-l border-zinc-800 text-zinc-500 hover:text-zinc-300 hover:bg-[#0c100e] transition-colors">
        ...
        <div className="flex items-center justify-center h-full px-4 border-l border-zinc-800 bg-[#0a0f0c]">
          {!userId ? (
            <SignInButton mode="modal">
              <button className="text-lime hover:text-lime/80 font-bold transition-colors">Sign In</button>
            </SignInButton>
          ) : (
